...
#define OFFSET_WIN_UNCLOAKTIME		1796	// 449 * 4	--->>> this offset sets the time when alien started uncloak (-1.0 when cloaked)
#define OFFSET_LIN_UNCLOAKTIME		1716	// (449+5) * 4	--->>> defines the difference between win32 to linux

#define OFFSET_WIN_CLOAK		1800	// 450 * 4	--->>> this offset is needed to check if player is "cloaked" / "cloak fade in/out" right now or not (int = cloaked or not // float = sequence of cloak fade in/out)
#define OFFSET_LIN_CLOAK		1820	// (450+5) * 4	--->>> defines the difference between win32 to linux
